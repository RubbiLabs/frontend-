"use client";
import React, { useEffect, useState } from "react";
import { useRouter } from "next/navigation";
import { Wallet, Mail, ArrowRight, Shield, Zap, CreditCard } from "lucide-react";
import { useWallet } from "../../context/WalletContext";
import { useSocialAuth } from "../../context/SocialAuthContext";
import { useToast } from "../../context/ToastContext";
import RubbiLogo from "../../components/ui/RubbiLogo";

export default function OnboardingPage() {
  const router = useRouter();
  const { isConnected, isHydrated, connect, isConnecting } = useWallet();
  const { loginWithGoogle, isSocialLogin, isSocialLoading } = useSocialAuth();
  const { success, error } = useToast();

  useEffect(() => {
    if (!isHydrated) return;
    if (isConnected || isSocialLogin) {
      router.replace("/dashboard");
    }
  }, [isConnected, isHydrated, isSocialLogin, router]);

  const handleWalletConnect = async () => {
    try {
      await connect();
      success("Wallet Connected!", "Redirecting to dashboard...");
      router.push("/dashboard");
    } catch {
      error("Connection Failed", "Please try again or install MetaMask.");
    }
  };

  const handleGoogleLogin = async () => {
    try {
      await loginWithGoogle();
      success("Signed in with Google!", "Redirecting to dashboard...");
      router.push("/dashboard");
    } catch (err: any) {
      error("Sign-in Failed", err.message || "Please try again.");
    }
  };

  if (!isHydrated || isConnected || isSocialLogin) return null;

  return (
    <div className="min-h-screen bg-neutral-50 font-manrope flex items-center justify-center p-6">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          <div className="flex items-center justify-center gap-2 mb-4">
            <RubbiLogo size={32} />
            <span className="text-2xl font-bold text-primary">
              <span className="text-3xl text-primary-400 font-extrabold">u</span>bbi
            </span>
          </div>
          <h1 className="text-2xl font-extrabold text-neutral-900">Welcome to Rubbi</h1>
          <p className="text-sm text-neutral-500 mt-2">Choose how you'd like to get started</p>
        </div>

        <div className="space-y-3">
          <button
            onClick={handleWalletConnect}
            disabled={isConnecting}
            className="w-full flex items-center gap-4 p-5 bg-white rounded-2xl border-2 border-neutral-200 hover:border-primary/40 hover:shadow-md transition-all group text-left"
          >
            <div className="w-12 h-12 bg-primary/10 rounded-xl flex items-center justify-center shrink-0">
              <Wallet size={22} className="text-primary" />
            </div>
            <div className="flex-1">
              <p className="font-bold text-neutral-900 group-hover:text-primary transition-colors">Connect Wallet</p>
              <p className="text-xs text-neutral-500 mt-0.5">MetaMask, Coinbase Wallet, or any Web3 wallet</p>
            </div>
            <ArrowRight size={18} className="text-neutral-300 group-hover:text-primary transition-colors" />
          </button>

          <button
            onClick={handleGoogleLogin}
            disabled={isSocialLoading}
            className="w-full flex items-center gap-4 p-5 bg-white rounded-2xl border-2 border-neutral-200 hover:border-primary/40 hover:shadow-md transition-all group text-left"
          >
            <div className="w-12 h-12 bg-white rounded-xl flex items-center justify-center shrink-0 border border-neutral-200">
              <svg width="20" height="20" viewBox="0 0 24 24">
                <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4"/>
                <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853"/>
                <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05"/>
                <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335"/>
              </svg>
            </div>
            <div className="flex-1">
              <p className="font-bold text-neutral-900 group-hover:text-primary transition-colors">Continue with Google</p>
              <p className="text-xs text-neutral-500 mt-0.5">Sign in with your Google account — no wallet needed</p>
            </div>
            <ArrowRight size={18} className="text-neutral-300 group-hover:text-primary transition-colors" />
          </button>

          <button
            onClick={handleGoogleLogin}
            disabled={isSocialLoading}
            className="w-full flex items-center gap-4 p-5 bg-white rounded-2xl border-2 border-neutral-200 hover:border-primary/40 hover:shadow-md transition-all group text-left"
          >
            <div className="w-12 h-12 bg-primary/10 rounded-xl flex items-center justify-center shrink-0">
              <Mail size={22} className="text-primary" />
            </div>
            <div className="flex-1">
              <p className="font-bold text-neutral-900 group-hover:text-primary transition-colors">Continue with Email</p>
              <p className="text-xs text-neutral-500 mt-0.5">Use your email address to create a wallet automatically</p>
            </div>
            <ArrowRight size={18} className="text-neutral-300 group-hover:text-primary transition-colors" />
          </button>
        </div>

        <div className="mt-8 grid grid-cols-3 gap-4">
          <div className="text-center">
            <div className="w-10 h-10 bg-primary/10 rounded-xl flex items-center justify-center mx-auto mb-2">
              <Shield size={16} className="text-primary" />
            </div>
            <p className="text-[10px] font-bold text-neutral-500 uppercase tracking-wider">Gasless</p>
          </div>
          <div className="text-center">
            <div className="w-10 h-10 bg-primary/10 rounded-xl flex items-center justify-center mx-auto mb-2">
              <Zap size={16} className="text-primary" />
            </div>
            <p className="text-[10px] font-bold text-neutral-500 uppercase tracking-wider">Instant</p>
          </div>
          <div className="text-center">
            <div className="w-10 h-10 bg-primary/10 rounded-xl flex items-center justify-center mx-auto mb-2">
              <CreditCard size={16} className="text-primary" />
            </div>
            <p className="text-[10px] font-bold text-neutral-500 uppercase tracking-wider">On-Chain</p>
          </div>
        </div>

        <p className="text-center text-[10px] text-neutral-400 mt-6">
          By continuing, you agree to Rubbi's Terms of Service and Privacy Policy.
        </p>
      </div>
    </div>
  );
}
